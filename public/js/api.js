$(document).ready(function () {
  ...
  let popularInterval, trendingInterval;

  function styleDynamicContent() {
    $('.carousel-item img').css('border', '2px solid #FFD700'); // Borde amarillo para imágenes dinámicas
}
  ...
  function initPopularCarousel(movies) {
      ...
      $('#popular-carousel').carousel('dispose'); // Detener el carrusel
      clearInterval(popularInterval); // Limpiar el intervalo existente
      ...
      for (let i = 0; i < movies.length; i += 5) {
          ...
          const row = $('<div class="row no-gutters">'); // Crear una fila para las imágenes, sin márgenes
      ...
      $('#popular-carousel').carousel({
          interval: false // Desactivar el intervalo automático de Bootstrap
      });

      // Configurar intervalo para avanzar automáticamente cada 8 segundos (8000 milisegundos)
      popularInterval = setInterval(avanzarCarrusel, 8000); // Cambiar el valor según la velocidad deseada
  }
  ...
  function initTrendingCarousel(movies) {
      ...
      $('#trending-carousel').carousel('dispose'); // Detener el carrusel
      clearInterval(trendingInterval); // Limpiar el intervalo existente
      ...
      // Configurar intervalo para avanzar automáticamente cada 8 segundos (8000 milisegundos)
      trendingInterval = setInterval(avanzarCarrusel, 8000); // Cambiar el valor según la velocidad deseada
  }
  ...
  // Agregar evento resize con debounce
